...
import { CheckCircle2, CreditCard, Landmark, LoaderCircle, Smartphone, WalletCards } from "lucide-react";
import { FormEvent, useState } from "react";
import { Link } from "wouter";

const methods = [
  { name: "فودافون كاش", note: "محفظة محلية", icon: Smartphone },
...
export default function Checkout() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!cart?.itemCount || !cart.id) {
      setNotice("أضف منتجًا واحدًا على الأقل قبل إتمام الطلب.");
      return;
    }
    if (!user) {
      setNotice("سجّل الدخول أولًا لحفظ طلبك ومتابعة حالته.");
      startLogin();
      return;
    }
