...
import { clearCheckoutProfile, loadCheckoutProfile, saveCheckoutProfile } from "@/lib/checkoutProfile";
import { trpc } from "@/lib/trpc";
import { BookOpen, CheckCircle2, ChevronLeft, CircleDollarSign, FileText, LoaderCircle, LockKeyhole, MessageCircle, ShoppingBag, Trash2, UserRound } from "lucide-react";
...
export default function DigitalBookDetails() {
  ...
  const buy = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!book || !user) return;
    const order = await purchase.mutateAsync({ bookId: book.id, customerName, customerPhone, paymentMethod });
    if (rememberDetails) {
      saveCheckoutProfile({ customerName, customerPhone });
      setSavedOnDevice(true);
    }
  ...
  return <StoreLayout><main className="container py-8">
    ...
    {showPurchase && <section ref={purchaseRef} id="إتمام-الشراء" className="mt-10 scroll-mt-24 rounded-3xl border border-[#d9d4ed] bg-[#faf9ff] p-6"><div className="flex items-start justify-between"><div><p className="text-xs font-bold text-[#50427b]">إتمام الطلب</p><h2 className="mt-1 text-2xl font-black text-[#173c37]">{book.title}</h2></div><Button type="button" variant="ghost" onClick={() => { setShowPurchase(false); setCreatedOrder(null); }}>إغلاق</Button></div>{!isFree && !createdOrder && <div className="mt-5 flex flex-col gap-3 rounded-2xl border border-[#b7e6c9] bg-[#f3fcf6] p-4 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm leading-6 text-[#205f39]">تفضّل التواصل قبل تسجيل الطلب؟ أرسل لنا رسالة عن هذا الكتاب وسنساعدك في إتمام السداد.</p><Button asChild variant="outline" className="shrink-0 rounded-xl border-[#25D366] text-[#157b40] hover:bg-white"><a href={purchaseWhatsappUrl} target="_blank" rel="noreferrer"><MessageCircle className="ml-2 h-4 w-4" />التواصل عبر واتساب</a></Button></div>}{createdOrder ? <div className="mt-5 rounded-2xl bg-white p-5"><CheckCircle2 className="h-7 w-7 text-[#2d7a51]" /><h3 className="mt-3 font-black text-[#173c37]">اكتملت مرحلة التنفيذ تلقائيًا</h3><p className="mt-2 text-sm leading-7 text-stone-600">{isFree ? "أُضيف الكتاب إلى مكتبتك مباشرة ويمكنك البدء في القراءة." : <>رقم الطلب: <strong>{createdOrder.orderNumber}</strong>. تم تجهيز طلب الكتاب الرقمي تلقائيًا؛ اختر الآن طريقة إتمام السداد.</>}</p>{isFree ? <Button asChild className="mt-5 rounded-xl bg-[#173c37]"><Link href="/مكتبتي">اذهب إلى مكتبتي</Link></Button> : <div className="mt-5 grid gap-3 sm:grid-cols-2"><Button asChild className="h-12 rounded-xl bg-[#b76f2c] font-extrabold hover:bg-[#9a5821]"><Link href={`/إثبات-الدفع/${createdOrder.id}`}><FileText className="ml-2 h-5 w-5" />رفع إثبات السداد الآن</Link></Button><Button asChild variant="outline" className="h-12 rounded-xl border-[#25D366] text-[#157b40] hover:bg-[#effcf3]"><a href={whatsappUrl} target="_blank" rel="noreferrer"><MessageCircle className="ml-2 h-5 w-5" />التواصل عبر واتساب</a></Button></div>}<Button asChild variant="ghost" className="mt-3 rounded-xl"><Link href="/طلباتي">عرض تفاصيل الطلب</Link></Button></div> : loading ? <p className="mt-5 text-sm text-stone-500">جارٍ التحقق من الحساب…</p> : !user ? <div className="mt-5 rounded-2xl bg-white p-5"><p className="text-sm text-stone-600">سجل الدخول أولًا لربط الكتاب بحسابك.</p><Button onClick={() => startLogin()} className="mt-4 rounded-xl bg-[#173c37]">تسجيل الدخول</Button></div> : <form onSubmit={buy} className="mt-5 grid gap-4 md:grid-cols-2"><Field label="الاسم"><Input value={customerName} onChange={event => setCustomerName(event.target.value)} required placeholder="الاسم الكامل" autoComplete="name" /></Field><Field label="رقم الهاتف"><Input value={customerPhone} onChange={event => setCustomerPhone(event.target.value)} required placeholder="01xxxxxxxxx" autoComplete="tel" inputMode="tel" dir="ltr" /></Field>{!isFree && <Field label="طريقة الدفع"><select value={paymentMethod} onChange={event => setPaymentMethod(event.target.value as (typeof paymentMethods)[number])} className="h-11 w-full rounded-xl border border-input bg-white px-3 text-sm">{paymentMethods.map(method => <option key={method}>{method}</option>)}</select></Field>}<div className="rounded-xl border border-[#d9d4ed] bg-white p-3 md:col-span-2"><label className="flex cursor-pointer items-start gap-2 text-sm text-stone-700"><input type="checkbox" checked={rememberDetails} onChange={event => setRememberDetails(event.target.checked)} className="mt-1 h-4 w-4 accent-[#50427b]" /><span><strong className="text-[#173c37]">حفظ بياناتي على هذا الجهاز</strong><br /><span className="text-xs text-stone-500">يُحفظ الاسم ورقم الهاتف في متصفحك فقط لتعبئة الشراء القادم بسرعة. لا تُحفظ بيانات الدفع.</span></span></label>{savedOnDevice && <Button type="button" variant="ghost" onClick={clearSavedDetails} className="mt-2 h-8 px-0 text-xs text-rose-700 hover:bg-transparent hover:text-rose-800"><Trash2 className="ml-1 h-3.5 w-3.5" />مسح البيانات المحفوظة من هذا الجهاز</Button>}</div><div className="flex items-end md:col-span-2"><Button type="submit" disabled={purchase.isPending} className="h-11 w-full rounded-xl bg-[#50427b]">{purchase.isPending && <LoaderCircle className="ml-2 h-4 w-4 animate-spin" />}{isFree ? "إضافة إلى مكتبتي" : "تسجيل الطلب والانتقال للسداد"}</Button></div>{purchase.error && <p className="md:col-span-2 text-sm text-rose-700">{purchase.error.message}</p>}</form>}</section>}
  </main></StoreLayout>;
}
