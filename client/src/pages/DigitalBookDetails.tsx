...
import { clearCheckoutProfile, loadCheckoutProfile, paymentMethods, PaymentMethod, saveCheckoutProfile } from "@/lib/checkoutProfile";
...
export default function DigitalBookDetails() {
  ...
  const buy = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!book || !user) return;
    const identity = validatePurchaseIdentity(customerName, customerPhone);
    if (!identity.ok) {
      toast.error(identity.message);
      return;
    }
    setCustomerName(identity.customerName);
    setCustomerPhone(identity.customerPhone);
    try {
      const order = await purchase.mutateAsync({ bookId: book.id, customerName: identity.customerName, customerPhone: identity.customerPhone, paymentMethod });
      if (rememberDetails) {
        saveCheckoutProfile({ customerName: identity.customerName, customerPhone: identity.customerPhone, paymentMethod });
        setSavedOnDevice(true);
        toast.success("تم حفظ بياناتك وطريقة الدفع على هذا الجهاز.");
      }
      setCreatedOrder({ id: order.id, orderNumber: order.orderNumber });
    } catch (error) {
      toast.error(formatSafePurchaseError(error));
    }
  };
