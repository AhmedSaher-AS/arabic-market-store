...
export default function DigitalBookDetails() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<PaymentMethod>("فودافون كاش");
  ...
  const seoDescription = (book.shortDescription || book.description || `اشترِ واقرأ كتاب ${book.title} من مكتبتك الرقمية.`).slice(0, 160);
  const productPath = `/كتب-رقمية/${encodeURIComponent(book.productHandle)}`;
  return <StoreLayout><SeoMeta title={`${book.title} | كتب رقمية | سوقك العربي`} description={seoDescription} canonicalPath={productPath} image={book.coverUrl} type="product" jsonLd={{ "@context": "https://schema.org", "@type": "Product", name: book.title, description: seoDescription, image: book.coverUrl || undefined, author: { "@type": "Person", name: book.author || "الناشر" }, inLanguage: book.language || "ar", offers: { "@type": "Offer", price: Number(book.price), priceCurrency: book.currencyCode, availability: book.isAvailable ? "https://schema.org/InStock" : "https://schema.org/OutOfStock", url: `${window.location.origin}${productPath}` } }} /><main className="container py-8">
