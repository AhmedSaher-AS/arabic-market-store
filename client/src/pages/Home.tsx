...
export default function Home() {
  ...
  const { data: storeSettings } = trpc.storeSettings.public.useQuery();
  return (
    <StoreLayout>
      <section className="relative isolate overflow-hidden border-b border-stone-200 bg-[#f5f1e8]">
        ...
        <div className="container relative grid items-center gap-10 py-12 lg:grid-cols-[1.08fr_0.92fr] lg:py-20">
          <div className="order-2 lg:order-1">
            <p className="mb-5 flex items-center gap-2 text-xs font-extrabold tracking-[0.18em] text-[#b76f2c]"><span className="h-px w-7 bg-[#b76f2c]" />{storeSettings?.heroEyebrow || "اختيارات منتقاة لك"}</p>
            <h1 className="max-w-2xl text-4xl font-black leading-[1.25] tracking-tight text-[#173c37] sm:text-5xl lg:text-6xl">{storeSettings?.heroTitle || "كل ما تحتاجه،"}<br /><span className="text-[#b76f2c]">{storeSettings?.heroHighlight || "في مكان عربي واحد."}</span></h1>
            <p className="mt-6 max-w-xl text-base leading-8 text-stone-600">{storeSettings?.heroDescription || "استكشف كتبًا ملهمة وملابس مختارة وأجهزة عملية عبر تجربة تسوق مصممة بوضوح وسهولة."}</p>
