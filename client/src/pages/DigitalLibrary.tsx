...
export default function DigitalLibrary() {
  ...
  const downloadBook = trpc.digitalBooks.download.useMutation({
    onSuccess: data => {
      window.open(data.downloadUrl, "_blank", "noopener,noreferrer");
      reader.refetch();
      toast.success(data.remainingDownloads === null ? "تم تجهيز رابط التنزيل المرخّص." : `تم تجهيز التنزيل. المتبقي: ${data.remainingDownloads}.`);
    },
    onError: error => toast.error(error.message || "تعذر إصدار رابط التنزيل."),
  });

  useEffect(() => {
    if (!selectedHandle && books[0]) setSelectedHandle(books[0].book.productHandle);
  }, [books, selectedHandle]);
  ...
  const requestDownload = () => {
    if (selectedHandle) downloadBook.mutate({ productHandle: selectedHandle });
  };
